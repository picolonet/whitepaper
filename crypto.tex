...
colluding between clients and a few replicas targting another replica
no data avail proof written to a blockchain in PoQ 
...
\end{remark}
\begin{definition}{\textbf{Semantic group:}}
...
\begin{definition}{\textbf{Access level $\mathcal{A}\ell$:}}
	Access level of a node is the probability with which its queries will be fulfilled by peers in its semantic group.
\end{definition}
\begin{definition}{\textbf{Cooperative:}}
	A node is said to be cooperative if it follows \cref{forward_rule}.
\end{definition}
...
definition2 fairness
rule 1 fulfilling query itself - PoQ servers as non-free rider proof
rule 2 sending queries to peers GET MSG with 2 hops

each node maintains coop score, free rider  and quality of results score
the score determines access level of the node

is verifying whether a node sent a response NP-HARD?
