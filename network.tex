...
\begin{theorem}[Fault Tolerance]
    ...
    For a content ID \(X\) with its set of home nodes given by \( H_X = HomeNodes(X) \) and \(S_n\) be the set of all
    nodes (of cardinality \(n\)) which satisfies the above properties, if \( |H_x| > 1 \) and \( \forall a, b \in S_n,
