...
% TODO:
% Describe the routing map. level by level. for a single lookup node. Diagram to illustrate an example routing map.
...
each digit \( [ 0, 1, 2..F\ \). At the \(i\) th level, the \( j\)th entry is the node with \( prefix(ID, i) + j\).
at where each level represents matching a prefix of that length (just like a routing table
for IPv6). As an example, Table \ref{table:routing_map} shows the different levels for the node with ID "691D".
Suppose we wish to route the ID "692B" given the routing table and we are doing a lookup at level 2. This means we
have a prefix match of "69" and are searching for a node in the column for the digit "2". This happens to be node
\(C_2\) and thus the query is then forwarded there. Similarly, suppose we are asked to route to node "615B". This would
be at level 1 since we have a match for the prefix "6". We would thus route to node \(B_1\) matching the column"1" and
row "6".
