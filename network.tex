...
Metadata and such, that is, any content that is content-addressable and needs to be part of the lookup layer.  
...
network. This is the case with all other content based DHTs as well \cite{Stoica_2001, Rowstron_2001, tapestry2004}. We
solve this problem by mapping each node \(y \in H_x \) to a \(Delegate(y)\) which is a node that exists on the network.
