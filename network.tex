...
\begin{itemize}
    \item {\em Publish(namespace, object ID, object, credentials)}: Makes the object available on the network, including first a copy on
    ...
    \item {\em Unpublish(namespace, object ID, credentials)}: Removes the object from the network making it inaccessible.
    \item {\em Lookup(namespace, object ID, credentials)}: Lookup item with the given object ID using the p2p lookup protocol
    ...
    \item {\em RemoteCall(namespace, node ID or object ID, credentials, method details)}: This executes a given functionality
        on a remote node. If the object ID is given, it first does a node lookup using the Lookup() method.
\end{itemize}
